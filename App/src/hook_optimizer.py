...
HOOK_PATTERNS = {
    ...
    "question": [
        r"^(est-ce que|pourquoi|comment|qu['']est-ce|savez-vous|vous savez|tu sais|c['']est quoi|what|why|how|do you know|did you know)",
        r"\?$",
...
class HookOptimizer:
    ...
    def analyze_hook(
        ...
        if power_count > 0:
            bonus = min(0.2, power_count * 0.05)
            score += bonus
    ...
    def optimize_moments(
        ...
        for moment in moments:
            ...
            if new_start != start and analysis.score >= self.min_hook_score:
                ...
                console.print(f"  [green]Hook optimisé:[/green] {start:.1f}s -> {new_start:.1f}s (score: {analysis.score:.2f})")
