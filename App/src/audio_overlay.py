...
DEFAULT_MUSIC_VOLUME: float = 0.15          # Volume par défaut de la musique (15%)
DEFAULT_DUCKING_RATIO: float = 0.3          # Ratio de volume pendant la parole (30%)
DEFAULT_SAMPLE_RATE: int = 44100            # Fréquence d'échantillonnage par défaut (Hz)
NORMALIZATION_HEADROOM: float = 0.9         # Marge avant clipping lors de la normalisation
MAX_NORMALIZATION_FACTOR: float = 2.0       # Facteur max de normalisation pour éviter d'amplifier le bruit
DUCKING_WINDOW_SECONDS: float = 0.05        # Fenêtre d'analyse du ducking (50ms)
FADE_DURATION_RATIO: int = 4               # Diviseur pour limiter la durée du fade par rapport à la durée totale
...
class AudioOverlayConfig:
    ...
    music_volume: float = DEFAULT_MUSIC_VOLUME  # 15% du volume original par défaut
    ...
    ducking_ratio: float = DEFAULT_DUCKING_RATIO  # Volume pendant la parole (30% du volume normal)
...
class AudioOverlay:
    ...
    def _normalize_audio(self, audio: AudioFileClip) -> AudioFileClip:
        ...
        fps = audio.fps or DEFAULT_SAMPLE_RATE
        ...
        if max_amplitude > 0:
            # Normaliser à NORMALIZATION_HEADROOM pour éviter le clipping
            normalize_factor = NORMALIZATION_HEADROOM / max_amplitude
            # Limiter la normalisation pour éviter d'amplifier le bruit
            normalize_factor = min(normalize_factor, MAX_NORMALIZATION_FACTOR)
    ...
    def _manual_fade_in(self, audio: AudioFileClip) -> AudioFileClip:
        ...
        fade_duration = min(self.config.fade_in_duration, audio.duration / FADE_DURATION_RATIO)
    ...
    def _manual_fade_out(self, audio: AudioFileClip) -> AudioFileClip:
        ...
        fade_duration = min(self.config.fade_out_duration, duration / FADE_DURATION_RATIO)
    ...
    def _apply_ducking(
        ...
        """
        fps = music.fps or DEFAULT_SAMPLE_RATE
        ...
        window_size = int(fps * DUCKING_WINDOW_SECONDS)  # Fenêtre de 50ms
