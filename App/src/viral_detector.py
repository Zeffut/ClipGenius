...
DEFAULT_SPECTRAL_THRESHOLD = 0.75       # Seuil par défaut pour changements spectraux
INTERVIEW_ENERGY_THRESHOLD = 0.55       # Seuil d'énergie pour interviews
INTERVIEW_ONSET_THRESHOLD = 0.45        # Seuil d'onsets pour interviews
INTERVIEW_SPECTRAL_THRESHOLD = 0.65     # Seuil spectral pour interviews
TUTORIAL_SILENCE_BOOST = 0.25           # Bonus silence pour tutoriels
...
class AudioAnalysisProfile:
    ...
    spectral_threshold: float = DEFAULT_SPECTRAL_THRESHOLD # Seuil pour changements spectraux
...
AUDIO_PROFILES: Dict[ContentType, AudioAnalysisProfile] = {
    ...
    ContentType.INTERVIEW: AudioAnalysisProfile(
        ...
        onset_weight=0.3,
        spectral_weight=0.5,
        energy_threshold=INTERVIEW_ENERGY_THRESHOLD,
        onset_threshold=INTERVIEW_ONSET_THRESHOLD,
        spectral_threshold=INTERVIEW_SPECTRAL_THRESHOLD,
        silence_boost=0.2,
    ...
    ContentType.TUTORIAL: AudioAnalysisProfile(
        ...
        onset_threshold=0.4,
        spectral_threshold=0.6,
        silence_boost=TUTORIAL_SILENCE_BOOST,
        min_score_adjustment=-0.1,
    ),
...
class ViralMomentDetector:
    ...
    def analyze(self, video_path: str) -> List[ViralMoment]:
        ...
        try:
            ...
            if duration < self.min_clip_duration:
                console.print(
                    f"[yellow]Vidéo trop courte ({duration:.0f}s"
                    f" < {self.min_clip_duration:.0f}s minimum)[/yellow]"
                )
                return []
    ...
    def _analyze_audio(self, audio_path: str, video_duration: float) -> List[Dict[str, Any]]:
        ...
        if profile.energy_weight < 0.5:
            console.print(
                f"[dim]Analyse audio adaptée pour {self.content_type.value}"
                f" (énergie pondérée à {profile.energy_weight:.0%})[/dim]"
            )
        ...
        try:
            ...
            if profile.onset_weight > 0.1:
                ...
                onset_density_normalized = (
                    (onset_density - onset_density.min())
                    / (onset_density.max() - onset_density.min() + 1e-8)
                )
            ...
            if profile.spectral_weight > 0.1:
                ...
                contrast_diff_normalized = (
                    (contrast_diff - contrast_diff.min())
                    / (contrast_diff.max() - contrast_diff.min() + 1e-8)
                )
